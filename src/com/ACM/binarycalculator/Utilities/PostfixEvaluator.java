 ...
 * 
 * Example: post-fix=6 2 + 5 * 8 4 / -
 * 
 * evaluation=38.0
 * 
 * ^^Notice that there is an additional ".0" appended to the number, if the
 * result is a whole number, so that should be rid of out side of this class. OR
 * make it so this function is smart enough to get rid of it for you.
 * 
...
public class PostfixEvaluator {
	 ...
	 * @return - The base-10 answer in the form of a string. If the answer is a
	 *         whole number it will return an additional decimal point and zero.
	 */
	public static String evaluate(String postfix) {
		...
		while (toke.hasMoreElements()) {
			...
			// we need to check if the the dash is being used as a minus
			// sign or
			// a negative sign.
			if (currentToken.equals("-")) {
				...
				// if we are dealing with a minus sign then do the math
				// stuff to
				// it
				else {
			...
			// numbers off of the stack and do math to it, then push the
			// outcome
			// back on the stack.
			else if (currentToken.equals("+") || currentToken.equals("x")
					|| currentToken.equals("-") || currentToken.equals("/")
